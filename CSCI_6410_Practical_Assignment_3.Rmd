...
zip_file <- "D:/CSCI_6410/Practical_Assignment_3/lab3_chest_xray.zip"

# Specify the destination directory to extract the files
destination_dir <- "D:/CSCI_6410/Practical_Assignment_3"
...
The lung illness known as pneumonia results in inflammation of the air sacs. when a result, breathing becomes challenging when the fluid-filled sacs expand. distinct viruses, bacteria, or even fungi can cause distinct kinds of pneumonia.

Automatically diagnosing pneumonia from X-rays has several advantages:

Faster diagnosis: Imaging the lungs quickly and painlessly is possible using X-rays. When compared to depending solely on radiologists, automatic analysis by computers could expedite the diagnosing process.

Increased accuracy: Computer analysis performed automatically has the potential to increase the accuracy of diagnosing pneumonia from X-rays, even in cases when radiologists possess exceptional skill.

Improved accessibility: Automatic analysis may be utilised as a primary screening method in places where access to medical personnel is restricted.
...
Yes, we can get some information about the pneumonia from the filenames of the X-rays that indicate the illness.  The labels "bacteria" and "virus" indicate the type of pneumonia they are.

This may increase the difficulty of predicting pneumonia in two ways:

Class Imbalance: One type of pneumonia (viral or bacterial) may be depicted in many more pictures than the other. The model may find it challenging to correctly identify the less common kind as a result.

Prioritise type over presence: Rather than concentrating on the specific aspects of the X-ray that point to pneumonia, the model may be trained to recognise the labels in the filename ("bacteria" or "virus").
...
Several attributes may require normalization prior to model training:

Brightness and Contrast: The pictures exhibit different brightness and contrast levels. The capacity of the model to learn consistent features may be impacted by this fluctuation. Normalization of brightness and contrast can guarantee that the model concentrates on the pertinent anatomical structures instead of the image illumination.

Size and Resolution: All of the photographs seem to be around the same size, but it's important to make sure that they're all the same size and resolution. This aids in the model's ability to interpret the photos consistently and extract characteristics.

Orientation and Alignment: It is important to standardize the X-rays' orientation. The model can detect pneumonia-specific patterns if all photos have the same orientation and the anatomical features are positioned identically.

Artifacts and Noise: Certain photos might have artifacts or noise, like the medical equipment shown in one of the X-rays. It is possible to stop the model from learning unimportant features by eliminating or minimizing noise and artifacts.

Cropping and padding: It may be required to make sure that the region of interest (the lungs) is centered and sized uniformly in every photograph. This can be accomplished by cropping out areas of the image that aren't needed and, if necessary, adding padding.

By normalizing these attributes, the dataset will become more consistent, which will help the model discover the pertinent characteristics linked to pneumonia more successfully.

# Data Pre-processing

The provided code segment presents a function called `process_images` that performs several pre-processing steps on the images. Here's an explanation of each pre-processing step and its purpose:
...
These methods can be applied if the dataset is unbalanced:

Oversampling: For the minority class, duplicate or make synthetic samples (e.g., using SMOTE).
Undersampling: To balance the dataset, eliminate samples from the majority class.
Data augmentation: To improve the variety of the minority class, apply transformations (such as rotation and flipping).
Class Weights: Modify the loss function to increase the minority class's weight.
Ensemble Methods: To increase minority class recognition, use strategies like boosting or bagging.
Synthetic Data Generation: To produce additional samples of the minority class, employ GANs.
...
In order to train a Convolu tional Neural Network (CNN), we will utilize the torch package in R. Torch is a powerful deep learning library that provides a wide range of functions and tools for building and training neural networks.
...
Although data augmentation is an effective tool, it has certain drawbacks:

Computational strain: When training a model with a large or sophisticated architecture, more data means more computational resources are required.

Noise Injection: Unadjusted augmentation procedures have the potential to inject noise or artificial distortions into the data. Imagine turning an image too much, producing absurd data that the model ignores in favour of important aspects.

Restricted Benefit for Varying Data: Data augmentation works best when the data's inherent variability mirror the changes that have been made. There may not be much benefit for tasks with naturally diverse data.

Overfitting Trap: If augmentation is done too aggressively, the model may overfit the augmented data, which may impair its performance on unobserved real-world data. It is important to strike a balance between realism and variance.

Bias Amplification: Augmentation can amplify preexisting biases in the original data. Even with augmentation, the model may have trouble processing X-rays of children if the majority of the training set consists of adult chest X-rays with pneumonia. 
...
ImageNet, also known as the "ImageNet Large Scale Visual Recognition Challenge 2012", is a massive image database used for research in computer vision and deep learning. Millions of labelled photos divided into thousands of classes are included.

According to estimates, ImageNet comprises over 14 million photos that are categorised into about 22,000 distinct groups.

ImageNet is useful for the following reasons when diagnosing pneumonia from X-rays:

Big, diverse dataset: Pre-trained models can learn general properties about objects and textures that might be helpful for various computer vision tasks, including medical image analysis, thanks to the enormous number of images and categories in ImageNet.

Foundation for transfer learning: Pre-trained models on ImageNet offer a solid base for transfer learning. Even with a smaller medical image dataset, researchers can fine-tune the model for specific tasks like pneumonia identification by utilising this pre-trained knowledge. When this is done instead of building a model from scratch, accuracy can be greatly increased and training time decreased.
...
The reasons I believe utilizing MobileNet can enhance the outcomes are as follows:

Decreased Computational Cost: Our assignment mentions limitations with resources and processing time. Compared to more intricate architectures, MobileNet is computationally less expensive to train since it is specifically made to be effective for mobile and embedded devices.

Faster Training: Given the time limits specified in our assignment, MobileNet's efficiency should allow it to train more quickly than a complex model.

Possibility of Accuracy with Smaller Datasets: Our assignment mentions a practical dataset might be used.  If the medical image dataset is not very large, MobileNets' reputation for high performance even with smaller datasets may prove advantageous.

**9_** How many parameters does this network have? How does this compare to better performing networks available in torch?

When compared to other high-performing networks in Torch, MobileNet is well-known for its efficiency and usually has a considerably less number of parameters.

Here is an explanation:

MobileNet: Depending on the version (MobileNetV1, MobileNetV2, etc.) and width multiplier selected, the number of parameters can change, although a standard MobileNet model may contain 3.4 million parameters.

High-Performing Networks: ResNet-50 and VGG-16 are two well-known examples of high-performing networks in Torch that can have a lot more parameters.  VGG-16 can have about 138 million parameters, and ResNet-50 can have about 25.6 million.

This significant difference in parameters contributes to MobileNet's advantages in terms of:

Computational Efficiency: MobileNet is faster to train and operate on devices with constrained resources because to its lower parameter count, which results in less computation being required during training and inference.

Potentially Faster Training: Compared to high-performing networks, MobileNet frequently requires less training time because it has fewer parameters to learn.

But it's crucial to keep in mind the compromise:

Possibility of Lower Accuracy: High-performing networks with more parameters can occasionally achieve even better accuracy on hard tasks, even though MobileNet can achieve good accuracy, particularly for smaller datasets.
